  ...
  if (joint) {
    ...
    switch (joint->type) {
      case urdf::Joint::FLOATING:
        ...
        visitor.addJointAndBody(UrdfVisitorBase::FLOATING, axis, parent_frame_id,
      ...
      case ::urdf::Joint::REVOLUTE:
        ...
        visitor.addJointAndBody(UrdfVisitorBase::REVOLUTE, axis, parent_frame_id,
                                joint_placement, joint->name, Y, link->name, max_effort,
                                max_velocity, min_config, max_config, friction,
                                damping);
        break;
      ...
      case ::urdf::Joint::CONTINUOUS:  // Revolute joint with no joint limits
        ...
        visitor.addJointAndBody(UrdfVisitorBase::CONTINUOUS, axis, parent_frame_id,
                                joint_placement, joint->name, Y, link->name, max_effort,
                                max_velocity, min_config, max_config, friction,
                                damping);
        break;
      ...
      case ::urdf::Joint::PRISMATIC:
        ...
        visitor.addJointAndBody(UrdfVisitorBase::PRISMATIC, axis, parent_frame_id,
                                joint_placement, joint->name, Y, link->name, max_effort,
                                max_velocity, min_config, max_config, friction,
                                damping);
        break;
...
template <typename DATATYPE>
void PinocchioModelTpl<DATATYPE>::init(const urdf::ModelInterfaceSharedPtr &urdfTree,
...
template <typename DATATYPE>
PinocchioModelTpl<DATATYPE>::PinocchioModelTpl(const std::string &urdf_filename,
                                               const Vector3 &gravity,
                                               const bool &verbose)
    : verbose(verbose) {
...
PinocchioModelTpl<DATATYPE>::PinocchioModelTpl(
    const urdf::ModelInterfaceSharedPtr &urdfTree, const Vector3 &gravity,
    const bool &verbose)
    : verbose(verbose) {
...
template <typename DATATYPE>
void PinocchioModelTpl<DATATYPE>::setJointOrder(const std::vector<std::string> &names) {
...
template <typename DATATYPE>
void PinocchioModelTpl<DATATYPE>::setLinkOrder(const std::vector<std::string> &names) {
...
void PinocchioModelTpl<DATATYPE>::printFrames(void) {
  print_info("Joint dim ", model.joints.size(), " ", model.nv, " ", model.nvs.size(),
             " ", model.idx_vs.size());
  print_info("Joint Tangent dim ", model.nq, " ", model.nqs.size(), " ",
             model.idx_qs.size());
  print_info("Joint Limit ", model.lowerPositionLimit.transpose(), " ",
             model.upperPositionLimit.transpose());
...
std::vector<std::string> PinocchioModelTpl<DATATYPE>::getChainJointName(
  ...
  for (auto index : index_pinocchio)
    if (joint_index_pinocchio2user[index] != -1) ret.push_back(model.names[index]);
  return ret;
}
...
template <typename DATATYPE>
void PinocchioModelTpl<DATATYPE>::computeForwardKinematics(const VectorX &qpos) {
...
Eigen::Matrix<DATATYPE, 6, Eigen::Dynamic> PinocchioModelTpl<DATATYPE>::getLinkJacobian(
  ...
  pinocchio::getJointJacobian(model, data, jointId, pinocchio::ReferenceFrame::WORLD,
                              J);
  if (local) {
...
PinocchioModelTpl<DATATYPE>::computeIKCLIK(const size_t &index, const Vector7 &pose,
...
std::tuple<Eigen::Matrix<DATATYPE, Eigen::Dynamic, 1>, bool,
           Eigen::Matrix<DATATYPE, 6, 1>>
PinocchioModelTpl<DATATYPE>::computeIKCLIKJL(const size_t &index, const Vector7 &pose,
                                             const VectorX &q_init,
...
Eigen::MatrixXd PinocchioModelTpl::computeGeneralizedMassMatrix(const Eigen::VectorXd
&qpos) { pinocchio::crba(model, data, posS2P(qpos));
    data.M.triangularView<Eigen::StrictlyLower>() =
            data.M.transpose().triangularView<Eigen::StrictlyLower>();
    return indexS2P.transpose() * data.M * indexS2P;
}

Eigen::MatrixXd PinocchioModelTpl::computeCoriolisMatrix(const Eigen::VectorXd &qpos,
const Eigen::VectorXd &qvel) { return indexS2P.transpose() *
           pinocchio::computeCoriolisMatrix(model, data, posS2P(qpos), indexS2P * qvel)
* indexS2P;
}
