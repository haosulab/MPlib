...
private:
    ...
    void init(urdf::ModelInterfaceSharedPtr const &urdfTree, std::string const &package_dir_);
    void dfs_parse_tree(urdf::LinkConstSharedPtr const& link, std::string);

public:

    FCLModelTpl(urdf::ModelInterfaceSharedPtr const &urdfTree, std::string const& package_dir, bool const &verbose = true, bool const& convex = false);

    FCLModelTpl(std::string const &urdf_filename, bool const &verbose = true, bool const& convex = false);

    inline std::vector<std::pair<size_t, size_t>> &getCollisionPairs() { return collision_pairs; }

    inline std::vector<CollisionObject_ptr> &getCollisionObjects() { return collision_objects; }

    inline std::vector<std::string> getCollisionLinkNames() { return collision_link_names; }

    inline std::vector<std::string> getUserLinkNames() { return user_link_names; }

    inline std::vector<size_t> getCollisionLinkUserIndices() { return collision_link_user_indices; }
    ...
    std::vector<fcl::CollisionResult<DATATYPE>> collideFull(
        CollisionRequest const& request=CollisionRequest(1, false, 1, false, true, fcl::GJKSolverType::GST_INDEP, 1e-6));
};
