...
KDLModelTpl<DATATYPE>::KDLModelTpl(std::string const &urdf_filename, std::vector<std::string> const &joint_names,
...
template<typename DATATYPE>
std::tuple<Eigen::Matrix<DATATYPE, Eigen::Dynamic, 1>, int> KDLModelTpl<DATATYPE>::chainIKLMA(size_t const &index, VectorX const & q0, Vector7 const &pose) {
    KDL::Chain chain;
    ...
    KDL::Frame frame_goal = KDL::Frame(KDL::Rotation::Quaternion(pose[4], pose[5], pose[6], pose[3]), KDL::Vector(pose[0], pose[1], pose[2]));
...
template<typename DATATYPE>
std::tuple<Eigen::Matrix<DATATYPE, Eigen::Dynamic, 1>, int> KDLModelTpl<DATATYPE>::chainIKNR(size_t const &index, VectorX const & q0, Vector7 const &pose) {
    KDL::Chain chain;
    ASSERT(index < user_link_names.size(), "link index out of bound");
    tree.getChain(tree_root_name, user_link_names[index], chain);
...
template<typename DATATYPE>
std::tuple<Eigen::Matrix<DATATYPE, Eigen::Dynamic, 1>, int> KDLModelTpl<DATATYPE>::chainIKNRJL(size_t const &index, VectorX const & q0, Vector7 const &pose,
...
template<typename DATATYPE>
std::tuple<Eigen::Matrix<DATATYPE, Eigen::Dynamic, 1>, int> KDLModelTpl<DATATYPE>::TreeIKNRJL(const std::vector<std::string> endpoints, 
                                                                                               VectorX const & q0, std::vector<Vector7> const &poses,
                                                                                               VectorX const & qmin, VectorX const & qmax) {
    ...
    for (int i = 0; i < endpoints.size(); i++) {
        frames[endpoints[i]] = KDL::Frame(KDL::Rotation::Quaternion(poses[i][4], poses[i][5], poses[i][6], poses[i][3]), KDL::Vector(poses[i][0], poses[i][1], poses[i][2]));
    }

    for (int i = 0; i < n; i++)
        q_init(i) = q0[joint_mapping_kdl_2_user[i]];
    
    KDL::TreeIkSolverPos_NR_JL solver(tree, endpoints, q_min, q_max, fkpossolver, ikvelsolver, 1000, 1e-6);

    auto retval = solver.CartToJnt(q_init, frames, q_sol);
    
    VectorX q1 = q0;
    for (int i = 0; i < n; i++) 
        q1[joint_mapping_kdl_2_user[i]] = q_sol(i);
