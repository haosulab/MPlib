...
#define DEFINE_TEMPLATE_URDF_UTILS(DATATYPE) \
    template pinocchio::InertiaTpl<DATATYPE, 0> convert_inertial<DATATYPE>(const urdf::Inertial &Y);\
    template pinocchio::InertiaTpl<DATATYPE, 0> convert_inertial<DATATYPE>(const urdf::InertialSharedPtr &Y);\
    template pinocchio::SE3Tpl<DATATYPE, 0> pose_to_se3<DATATYPE>(const urdf::Pose &M); \
    template Eigen::Transform<DATATYPE, 3, Eigen::Isometry> se3_to_transform<DATATYPE>(const pinocchio::SE3Tpl<DATATYPE, 0> &T);\
    template Eigen::Transform<DATATYPE, 3, Eigen::Isometry> pose_to_transform<DATATYPE>(const urdf::Pose &M);\
    template pinocchio::SE3Tpl<DATATYPE, 0> transform_to_se3<DATATYPE>(const Eigen::Transform<DATATYPE, 3, Eigen::Isometry> &T);\
    template std::shared_ptr<fcl::BVHModel<fcl::OBBRSS<DATATYPE>>> load_mesh_as_BVH<DATATYPE>(const std::string &mesh_path, const Eigen::Matrix<DATATYPE, 3, 1> &scale); \
    template std::shared_ptr<fcl::Convex<DATATYPE>> load_mesh_as_Convex<DATATYPE>(const std::string &mesh_path, const Eigen::Matrix<DATATYPE, 3, 1> &scale);
...
Eigen::Transform<DATATYPE, 3, Eigen::Isometry> pose_to_transform(const urdf::Pose &M) {
    ...
    Eigen::Transform<DATATYPE, 3, Eigen::Isometry> ret = Eigen::Transform<DATATYPE, 3, Eigen::Isometry>::Identity();
...
pinocchio::SE3Tpl<DATATYPE, 0> pose_to_se3(const urdf::Pose &M) {
    const urdf::Vector3 &p = M.position;
    const urdf::Rotation &q = M.rotation;
    return pinocchio::SE3Tpl<DATATYPE>(Eigen::Quaternion<DATATYPE>(q.w, q.x, q.y, q.z).matrix(),
...
pinocchio::InertiaTpl<DATATYPE, 0> convert_inertial(const urdf::Inertial &Y) {
    ...
    const Eigen::Matrix<DATATYPE, 3, 3> &R = Eigen::Quaternion<DATATYPE>(q.w, q.x, q.y, q.z).matrix();
...
void AssimpLoader::load(const std::string &file_name) {
    ...
    if (!scene) {
        const std::string exception_message(std::string("Could not load resource ") + file_name + std::string("\n") +
...
template<typename DATATYPE>
int dfs_build_mesh(const aiScene *scene, const aiNode *node, const Eigen::Matrix<DATATYPE, 3, 1> &scale,
                   int vertices_offset,
                   std::vector<fcl::Vector3<DATATYPE>> &vertices, std::vector<fcl::Triangle> &triangles) {
    if (!node) return 0;
    ...
    for (uint32_t i = 0; i < node->mNumChildren; ++i)
        nbVertices += dfs_build_mesh(scene, node->mChildren[i], scale, nbVertices, vertices, triangles);
    return nbVertices;
}
...
load_mesh_as_BVH(const std::string &mesh_path, const Eigen::Matrix<DATATYPE, 3, 1> &scale) {
    ...
    int nbVertices = dfs_build_mesh<DATATYPE>(loader.scene, loader.scene->mRootNode, scale, 0, vertices, triangles);
    //std::cout << "Num of vertex " << nbVertices << " " << vertices.size() << " " << triangles.size() << std::endl;
...
KDL::Joint toKdl(urdf::JointSharedPtr jnt) {
    ...
    switch (jnt->type) {
        ...
        case urdf::Joint::REVOLUTE:
            return KDL::Joint(jnt->name, F_parent_jnt.p, F_parent_jnt.M * toKdl(jnt->axis), KDL::Joint::RotAxis);
        case urdf::Joint::CONTINUOUS:
            return KDL::Joint(jnt->name, F_parent_jnt.p, F_parent_jnt.M * toKdl(jnt->axis), KDL::Joint::RotAxis);
        case urdf::Joint::PRISMATIC:
            return KDL::Joint(jnt->name, F_parent_jnt.p, F_parent_jnt.M * toKdl(jnt->axis), KDL::Joint::TransAxis);
        default:
...
bool addChildrenToTree(const urdf::LinkConstSharedPtr &root, KDL::Tree &tree, bool const &verbose) {
    ...
    for (size_t i = 0; i < children.size(); i++)
        if (!addChildrenToTree(children[i], tree, verbose))
            return false;
    return true;
}

bool treeFromUrdfModel(const urdf::ModelInterfaceSharedPtr &robot_model, KDL::Tree &tree, std::string &tree_root_name, bool const &verbose) {
