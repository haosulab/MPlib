...
class Planner(object):
	def __init__(
		self,
		urdf="/root/ocrtoc_ws/src/sapien_simulator/urdf/ur5e_sapien.urdf",
		srdf="/root/ocrtoc_ws/src/sapien_simulator/urdf/ur5e_sapien.srdf",
		# used to align the order of user links
		user_link_names=['world', 'table', 'base_link', 'kinect_camera_visor',
						'kinect_camera_base', 'kinect_camera_body', 'base',
						'shoulder_link', 'upper_arm_link', 'forearm_link',
						'wrist_1_link', 'wrist_2_link', 'wrist_3_link',
						'realsense_camera_link', 'realsense_usb_plug_link',
						'realsense_camera_bottom_screw_frame', 'tool0',
						'robotiq_ur_coupler', 'robotiq_2f_85_base',
						'robotiq_2f_85_right_pad', 'robotiq_2f_85_right_follower',
						'robotiq_2f_85_left_pad', 'robotiq_2f_85_left_follower',
						'robotiq_arg2f_base_link', 'robotiq_2f_85_right_spring_link',
						'robotiq_2f_85_right_driver', 'robotiq_2f_85_right_coupler',
						'robotiq_2f_85_left_spring_link', 'robotiq_2f_85_left_driver',
						'robotiq_2f_85_left_coupler', 'ee_link'],
		# used to align the order of user links
		user_joint_names=['shoulder_pan_joint', 'shoulder_lift_joint', 'elbow_joint',
						'wrist_1_joint', 'wrist_2_joint', 'wrist_3_joint',
						'robotiq_2f_85_right_driver_joint', 'robotiq_2f_85_left_driver_joint',
						'robotiq_2f_85_right_spring_link_joint', 'robotiq_2f_85_right_driver_mimic_joint',
						'robotiq_2f_85_left_spring_link_joint', 'robotiq_2f_85_left_driver_mimic_joint'],
		move_group="ee_link",
		move_group_joint_names = ["shoulder_pan_joint", "shoulder_lift_joint", "elbow_joint",
									"wrist_1_joint", "wrist_2_joint", "wrist_3_joint"],
		joint_vel_limits=[0.78, 0.78, 0.78, 0.78, 0.78, 0.78],
		joint_acc_limits=[0.78, 0.78, 0.78, 0.78, 0.78, 0.78]
	):
		self.urdf = urdf
		self.srdf = srdf
		...
		self.move_group_joint_names = move_group_joint_names
		...
		assert(len(self.move_group_joint_names) == len(self.move_group_joint_indices))
	...
	def IK(self, goal_pose, start_qpos, n_init_qpos = 20, threshold = 1e-3):
		...
		for i in range(n_init_qpos):
			ik_results = self.pinocchio_model.compute_IK_CLIK_JL(index, goal_pose, start_qpos, 
	...
	def plan(self, goal_pose, current_qpos, rrt_range = 0.2, planning_time = 1, 
				step = 0.1, fix_joint_limits = True, use_point_cloud = False, use_attach = False, verbose = False):
		...
		if status == "Exact solution":
			...
			times, pos, vel, acc, duration = self.TOPP(path, step)
