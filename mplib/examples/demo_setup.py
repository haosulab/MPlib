...
class DemoSetup:
    ...
    def follow_path(self, result):
        ...
        # this makes sure the robot stays neutrally boyant instead of sagging under gravity
        for i in range(n_step):
            qf = self.robot.compute_passive_force(
                external=False, gravity=True, coriolis_and_centrifugal=True
            )
            self.robot.set_qf(qf)
            # set the joint positions and velocities for move group joints only. The others are not the responsibility of the planner
    ...
    def move_to_pose_with_RRTConnect(
        ...
        """
        # result is a dictionary with keys 'status', 'time', 'position', 'velocity', 'acceleration', 'duration'
