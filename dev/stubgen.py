        ...
        if validate:
            ...
            try:
                ...
                if FunctionSignature.signature_downgrade:
                    ...
                    logger.log(
                        lvl,
                        "Generated stubs signature is degraded to `(*args, **kwargs) ->"
                        " typing.Any` for",
                    )
                else:
                    lvl = logging.WARNING
...
class StubsGenerator(object):
    ...
    GLOBAL_CLASSNAME_REPLACEMENTS = {
        ...
        re.compile(
            r"(?<!\w)(?P<type>Callable|Dict|[Ii]terator|[Ii]terable|List|Optional|Set|Tuple|Union')(?!\w)"
...
class ModuleStubsGenerator(StubsGenerator):
    ...
    def to_lines(self):  # type: () -> List[str]
        ...
        # ---------------------------------------------------------------------------- #
        # NOTE(jigu): Add the following codes to deal with extension submodules
        # I use a simple way to distinguish extension from pure python module
        # More detail analysis can be found at https://stackoverflow.com/questions/20339053/in-python-how-can-one-tell-if-a-module-comes-from-a-c-extension.
        from importlib.machinery import EXTENSION_SUFFIXES

        if (
            hasattr(self.module, "__file__")
            and os.path.splitext(self.module.__file__)[-1] in EXTENSION_SUFFIXES
        ):
            result.extend(["from . import {}".format(k) for k in sorted(all_)])
            result += [""]
        # ---------------------------------------------------------------------------- #
        result.append(
            "__all__ = [\n    "
...
def main(args=None):
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Render `numpy.ndarray` without (non-standardized) bracket-enclosed type"
            " and shape info"
        ),
    )
    parser.add_argument(
